...
De acordo com Freitas (\citeyear{de2013metodologia}), sobre a natureza da pesquisa, ela pode ser:
...
\begin{figure}[H]
    ...
    \caption[Fluxo de atividades da primeira etapa (TCC1)]{Fluxo de atividades da primeira etapa (TCC1). Fonte: Autor.}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{figuras/proposta/fluxo_atividade_tcc2.png}
    \caption[Fluxo de atividades da segunda etapa do projeto (TCC2)]{Fluxo de atividades da segunda etapa do projeto (TCC2). Fonte: Autor.}
...
\begin{itemize}
    ...
    \item \textbf{Desenvolver o sistema de recomendação:} implementar o sistema de recomendação usando as três abordagens estudadas (\textit{collaborative filtering, content-based filtering e critiquing-based}), sendo duas delas elaboradas com aprendizado de máquina. Será implementada a parte de virtualização de ambiente completo, tanto do ambiente de desenvolvimento quanto de produção, a integração contínua e o \textit{deploy} (entrega) automático;
    
    \item \textbf{Testar e realizar manutenção:} após desenvolvimento da 

    \item \textbf{Averiguar resultados obtidos:} será feito a análise dos resultados após os testes e manutenções que forem necessárias;
...
Para obter um conhecimento inicial sobre o projeto, foi levantado as seguintes \textit{features} (características), \textit{user stories} (estórias de usuário) e \textit{technical stories} (estória técnica):

\begin{itemize}
    \item Características (\textit{Features}):
    \begin{itemize}
        \item \textbf{FE01 - Recomendador baseado em crítica:} possibilitará o usuário receber recomendações de imóveis baseadas em críticas sobre características de imóveis, feitas pelo próprio usuário;

        \item \textbf{FE02 - Recomendador baseado em apredizado de máquina \textit{machine learning}:} permitirá o usuário receber recomendações de imóveis baseado nas ações dele no sítio virtual.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{FE01US02 - Alterar perfil de busca:} eu, como usuário, desejo alterar o perfil de busca para o ajustar as preferência e receber recomendações mais precisas;
        ...
        \item \textbf{FE02US05 - Visualizar recomendações na página do imóvel:} eu, como usuário, desejo visualizar recomendações de imóveis baseadas nas minhas ações no sítio virtal na página de detalhes do imóvel para diminuir o tempo de procura do imóvel ideal para mim;
    \end{itemize}
    
    \item Estórias técnicas (\textit{Technical stories}):
    \begin{itemize}
        \item \textbf{FE02TS01 - Registrar ações do usuário no sítio virtual:} eu, como desenvolvedor, desejo registrar as ações do usuário no site para poder predizer quais são os imóveis mais prováveis da experiência do usuário.
    \end{itemize}
\end{itemize}
...
\textbf{Time desenvolvimento}. Serão as pessoas que farão a implementação do produto, fazendo entregas a cada \textit{Sprint} \cite{Sutherland}. Esse papel será desempenhado pelos orientados (estudantes envolvidos no TCC), por possuírem conhecimento técnico sobre as tecnologias, metodologias, além do contexto do trabalho.

\subsubsection{Fluxo de desenvolvimento}

Definido as metodologias de desenvolvimento que serão utilizadas na segunda etapa (TCC2) e os artefatos que serão usados, a seguir na Figura \ref{fig:fluxo_scrum} pode ser visto o planejamento do fluxo de atividades que ocorrerá nessa etapa.
...
\begin{itemize}
    \item \textbf{Identificar \textit{features} (características):} verificar quais são os módulos de requisitos do projeto existentes.
    
    \item \textbf{Identificar \textit{user stories} (estórias de usuário) e \textit{technical stories} (estórias técnicas):} identificar as histórias de cada \textit{feature} (característica) definida anteriormente e prioriza-las com o nível de importância para o cliente;
    
    \item \textbf{\textit{Sprint planning} (planejamento do ciclo) :} planejar quais histórias serão desenvolvidas na \textit{Sprint}, sendo seguida a ordem de prioridade descrita no \textit{product backlog} (lista de funcionalidades do produto);
    
    \item \textbf{Implementar estórias:} desenvolver as estórias planejadas para aquela \textit{Sprint} até a conclusão de cada funcionalidade;
...
Testar e realizar manutenção &  &  &  & X & X \\ \hline
...
\begin{figure}[H]
    ...
    \caption[Arquitetura geral do sistema divida em serviços]{Arquitetura geral do sistema divida em serviços. Fonte: Autor.}
    \label{fig:sr_nivel2}
\end{figure}

A arquitetura existente no sistema da Liva segue o estilo Cliente-Servidor, com uma abordagem monolítica. Seu cliente é desenvolvido na linguagem JavaScript e com o \textit{framework} React. Sua API (Application Programming Interface - Interface de Programação de Aplicação) na linguagem Ruby com suporte do \textit{framework} Ruby on Rails, é integrado a um sistema gerenciador de banco de dados PostgreSQL.
...
\begin{sidewaysfigure}
    ...
    \caption[Arquitetura do módulo de Aprendizado de Máquina]{Arquitetura do módulo de Aprendizado de Máquina. Fonte: Autor.}
    \label{fig:sr_nivel3}
\end{sidewaysfigure}
...
\end{tabular}
\caption[Exemplo de conjunto de dados de treino]{Exemplo de conjunto de dados de treino. Fonte: Autor.}
\end{table}
...
\begin{figure}[H]
    ...
    \caption[Diagrama de Entidade-Relacionamento (DE-R)]{Diagrama de Entidade-Relacionamento (DE-R). Fonte: Autor.}
    \label{fig:der}
\end{figure}
...
A tabela dos dados do modelo de aprendizado de máquina serão populadas com dados que serão coletados do início do semestre de 2020 até a conclusão do sistema em no final do mesmo semestre, com o fim de se gerar um conjunto de dados (\textit{dataset}) de treino inicial.
